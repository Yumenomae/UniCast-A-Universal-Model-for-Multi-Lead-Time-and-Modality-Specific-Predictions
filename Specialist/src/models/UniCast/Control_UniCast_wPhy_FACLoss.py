...
class ModelInterface(LightningModule):
    ...
    def training_step(self, batch: Any, batch_idx: int):
        ...
        norm_preds = self.reverse_inp_transform(norm_preds)
        targets = self.reverse_inp_transform(targets)
        
        direct_loss = self.fourier_loss(norm_preds, targets, exist_cmpa)  # CMPA only
        self.log(
            "train/direct_loss",
            direct_loss,
            on_step=False,
        ...
        autoreg_loss = self.PhyAst_fourier_loss(assist_preds, target_assist, assists_exist_cmpa)
        self.log(
            "train/autoreg_loss",
            autoreg_loss,
            on_step=False,
    ...
    def evaluate(
        ...
        for target_lead_time in val_lead_times:
            ...
            norm_pred = self.reverse_inp_transform(norm_pred)
            targets = self.reverse_inp_transform(targets)

            FAL_loss, FCL_loss = self.fourier_loss.forward_fal_fcl(norm_pred, targets, exist_cmpa)  # CMPA only
            # Phy_FAL_loss, Phy_FCL_loss = self.fourier_loss.forward_fal_fcl(assist_pred, target_assist, assists_exist_cmpa)

            loss_per_lead_time = loss_per_lead_time + FAL_loss + FCL_loss
            # loss_per_lead_time = loss_per_lead_time + Phy_FAL_loss + Phy_FCL_loss
    ...
    def configure_optimizers(self):
        ...
        self.fourier_loss = RandomScheduling(total_step=self.hparams.max_epochs * n_steps,
                                             micro_batch=1,
                                             )
        
        self.PhyAst_fourier_loss = RandomScheduling_PhyAst(total_step=self.hparams.max_epochs * n_steps,
                                             micro_batch=1,
                                             ) 
