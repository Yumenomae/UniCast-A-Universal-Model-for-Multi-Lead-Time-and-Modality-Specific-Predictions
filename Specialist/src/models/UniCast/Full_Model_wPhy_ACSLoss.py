...
class ModelInterface(LightningModule):
    ...
    def __init__(
        self,
        pretrained_net: torch.nn.Module,
        finetuned_net: torch.nn.Module,
        net_ckpt_path: str,
        decoder_only: bool,
        ...
        self.save_hyperparameters(ignore=['pretrained_net', 'finetuned_net', 'net_ckpt_path'], logger=False)

        self.pretrained_net = pretrained_net
        self.finetuned_net = finetuned_net

        self.load_pretrained_weights_net(net_ckpt_path) 

        for p in self.pretrained_net.parameters():
            p.requires_grad = False

        if decoder_only:
            for param in finetuned_net.parameters():
                param.requires_grad = False

            # Unfreeze only the target layers
            for layer in [finetuned_net.layer3, finetuned_net.layer4]:
                for param in layer.parameters():
                    param.requires_grad = True
        
        self.ACSLoss = ACSLoss()
    #     if pretrained_path is not None:
    #         self.load_pretrained_weights(pretrained_path)
    
    def load_pretrained_weights_net(self, pretrained_path):
        ...
        msg = self.pretrained_net.load_state_dict(state_dict)
        print(msg)

        msg = self.finetuned_net.load_state_dict(state_dict)
        print(msg)
    ...
    def set_lat_lon(self, lat, lon):
        self.lat = lat
        self.lon = lon
        self.assist_compute = compute_air_column_variable_taking_geography(pressure_level=DEFAULT_PRESSURE_LEVELS,
                                                          lat=lat,
                                                          lon=lon,
    ...
    def pad(self, x: torch.Tensor):
        ...
        if h % self.finetuned_net.patch_size == 0 and w % self.finetuned_net.patch_size == 0:
            padded_x = x
            pad_size = 0
            return padded_x, pad_size
        
        # Calculate the pad size for the height if it's not divisible by the patch size
        if h % self.finetuned_net.patch_size != 0:
            pad_size = (self.finetuned_net.patch_size - h % self.finetuned_net.patch_size) // 2
        else: pad_size = 0
    ...
    def forward(self, x: torch.Tensor, hour_of_year: torch.Tensor, lead_time, variables) -> torch.Tensor:
        ...
        pred = self.finetuned_net(padded_x, hour_of_year, lead_time, variables)
        return pred[:, :, pad_size:-pad_size]
    
    def forward_pretrained(self, x: torch.Tensor, hour_of_year: torch.Tensor, lead_time, variables) -> torch.Tensor:
        """Perform a forward pass through the model `self.net`.
        ...
        """
        padded_x, pad_size = self.pad(x)
        pred = self.pretrained_net(padded_x, hour_of_year, lead_time, variables)
        return pred[:, :, pad_size:-pad_size]

    def forward_pretrained_val(self, x: torch.Tensor, hour_of_year: torch.Tensor, lead_time, variables):
        ...
        for sub_lead_time in base_lead_time:
            ...
            for _ in range(steps):
                norm_pred_diff = self.forward_pretrained(x, hour_of_year, sub_lead_time, variables) # diff in the normalized space
    ...
    def forward_train(
        ...
        for lead_time in train_lead_times[0]:
            ...
            pred_x = self.forward_pretrained_val(x, hour_of_year, int(lead_time), variables)
        ...
        # n_steps = train_lead_times.shape[-1]

        # # x is always in the normalized input space
        # for i in range(n_steps):
        #     norm_pred_diff = self(x, hour_of_year, train_lead_times[:, i], variables) # diff in the normalized space
        #     norm_pred = x + norm_pred_diff # prediction in the normalized space
        #     norm_preds.append(norm_pred)
    ...
    def forward_multi_step_validation(self, x: torch.Tensor, hour_of_year: torch.Tensor, lead_time, one_step_lead_time, variables):
        ...
        x = self.forward_pretrained_val(x, hour_of_year, prepare_lead_time, variables)
    ...
    def evaluate(
        ...
        for target_lead_time in val_lead_times:
            ...
            # norm_pred_wo_control = self.forward_wo_control_val(x, hour_of_year, target_lead_time, variables)
            # assist_pred = self.forward_assist(norm_pred_wo_control, (hour_of_year+target_lead_time)%8784, variables) # (B, 2, H, W)
            # assist_pred = self.assist_transform(assist_pred)

            targets = dict_y[target_lead_time]
            exist_cmpa = dict_exist_cmpa[target_lead_time]

            # assists = dict_assists[target_lead_time+1]
            # assists_exist_cmpa = dict_assists_exist_cmpa[target_lead_time + 1]

            # assists = self.reverse_inp_transform(assists)
            # fute_R = assists[:, 0]

            # target_dict = self.assist_compute.pressure_level_reshape_denorm(targets, self.reverse_inp_transform)
            # u, v, Qv = target_dict['u'], target_dict['v'], target_dict['Qv']
            # target_ac_h = self.assist_compute.predict_air_column_humidity(Qv, u, v, fute_R)

            # target_assist = target_ac_h.unsqueeze(1)
            # target_assist = self.assist_transform(target_assist)

            direct_loss = self.ACSLoss(norm_pred, targets, exist_cmpa)  # CMPA only
            # autoreg_loss = self.ACSLoss(assist_pred, target_assist,  assists_exist_cmpa)

            loss_per_lead_time = loss_per_lead_time + direct_loss
        ...
        target_loss_dict = {f"{stage}/ACSLoss_mean_over_leadtime": loss_per_lead_time / len(val_lead_times)} 
        self.log_dict(
            target_loss_dict,
            on_step=False,
            on_epoch=True,
