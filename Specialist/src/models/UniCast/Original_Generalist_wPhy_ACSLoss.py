...
class ModelInterface(LightningModule):
    ...
    def __init__(
        ...
        self.save_hyperparameters(ignore=['finetuned_net', 'net_ckpt_path'], logger=False)
    ...
    def forward_train(
        ...
        for lead_time in train_lead_times[0]:
            ...
            assist_pred = self.forward_assist(norm_pred, (hour_of_year+lead_time)%8784, variables)
    ...
    def forward_multi_step_validation(self, x: torch.Tensor, hour_of_year: torch.Tensor, lead_time, variables):
        # x: initial condition, B, V, H, W
        # variables: list of variable names
        # lead_time: scalar value, e.g., 168, use the same interval across the batch
        ...
        for sub_lead_time in base_lead_time:
            ...
            for _ in range(steps):
                norm_pred_diff = self.forward_one_step_validation(x, hour_of_year, sub_lead_time, variables) # diff in the normalized space
                x = x + norm_pred_diff # prediction in the normalized space
