...
class ModelInterface(LightningModule):
    ...
    def forward_assist(
        ...
        vars_dict = self.assist_compute.pressure_level_reshape_denorm(x, self.reverse_inp_transform)
        u, v, Qv = vars_dict['u'], vars_dict['v'], vars_dict['Qv']

        pred_x = self.forward_one_step_validation(x, hour_of_year, 1, variables)
        pred_x = self.reverse_inp_transform(pred_x)
        pred_R = pred_x[:, 0]

        pred_ac_h = self.assist_compute.predict_air_column_humidity(Qv, u, v, pred_R)
