...
class TimestepEmbedder(nn.Module):
    ...
    def forward(self, x, t):
        return self.mlp(t.unsqueeze(-1).to(device=x.device, dtype=x.dtype))
...
class Stormer(nn.Module):
    def __init__(self, 
        ...
        if in_img_size[1] % patch_size != 0:
            pad_size = patch_size - in_img_size[1] % patch_size
            in_img_size = (in_img_size[1] + pad_size, in_img_size[1])
    ...
    def forward(self, x, hour_of_year, lead_times, variables):
        ...
        for block in self.blocks:
            x = block(x, lead_times_emb)
        
        x = self.head(x, lead_times_emb)
        x = self.unpatchify(x)
