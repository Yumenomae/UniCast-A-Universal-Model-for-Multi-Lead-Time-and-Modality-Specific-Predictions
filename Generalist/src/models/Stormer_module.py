...
from src.models.components.Stormer import model
...
class ModelInterface(LightningModule):
    ...
    def pad(self, x: torch.Tensor):
        ...
        if h % self.net.patch_size == 0 and w % self.net.patch_size == 0:
            padded_x = x
            pad_size = 0
        ...
        if h % self.net.patch_size != 0:
            pad_size = (self.net.patch_size - h % self.net.patch_size) // 2
        else: pad_size = 0
    ...
    def forward(self, x: torch.Tensor, hour_of_year: torch.Tensor, lead_time, variables) -> torch.Tensor:
        ...
        return self.net(padded_x, hour_of_year, lead_time, variables)[:, :, pad_size:-pad_size]
    ...
    def forward_validation(self, x: torch.Tensor, hour_of_year: torch.Tensor, lead_time, variables):
        # x: initial condition, B, V, H, W
        # variables: list of variable names
        ...
        for sub_lead_time in base_lead_time:
            ...
            for _ in range(steps):
                norm_pred_diff = self(x, hour_of_year, sub_lead_time, variables) # diff in the normalized space
    ...
    def evaluate(
        ...
        for target_lead_time in val_lead_times:
            # all_norm_preds = []
            norm_pred = self.forward_validation(x, hour_of_year, target_lead_time, variables)
        ...
        # print(f'loss: {hour_of_year}_{loss_per_lead_time / len(val_lead_times)}')
        # print(f'pred: {norm_pred.mean()}')
